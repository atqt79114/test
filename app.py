...
# 頁面設定
# -------------------------------------------------
st.set_page_config(page_title="股票策略篩選器（防封鎖增強版）", layout="wide")
st.title("📈 股票策略篩選器（防封鎖增強版）")
...
def download_daily(ticker):
    ...
    for attempt in range(max_retries):
        try:
            # 隨機延遲 0.2 ~ 0.8 秒，模擬人類行為
            time.sleep(random.uniform(0.2, 0.8))
            
            # threads=False 是避免同時發送大量請求的關鍵
