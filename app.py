...
# 頁面設定
# -------------------------------------------------
st.set_page_config(page_title="股票策略篩選器（極速批量版）", layout="wide")
st.title("📈 股票策略篩選器（極速批量版）")
...
def download_batch_data(tickers_batch):
    """
    一次下載多檔股票資料，回傳 dict {ticker: dataframe}
    """
    try:
        # yfinance 支援一次傳入 list 進行批量下載
        # threads=True 開啟多執行緒加速 (批量下載時比較不會被擋)
        data = yf.download(tickers_batch, period="2y", interval="1d", group_by='ticker', progress=False, threads=True)
        
        result_dict = {}
        
        # 如果只有一檔，yfinance 結構不同，需特殊處理
        ...
        for t in tickers_batch:
            # 檢查該股票是否有資料 (有些可能下市或代碼錯誤)
            try:
                df = data[t].copy()
                # 簡單檢查：如果 Close 全是 NaN，視為無資料
                if df['Close'].isnull().all():
                    continue
                df = df.dropna(how='all') # 移除全空的行
                if not df.empty:
                    result_dict[t] = df
            except KeyError:
                continue
...
def strategy_smc_support(ticker, name, df, backtest_months):
    try:
        ...
        if (max(ma_vals) - min(ma_vals)) / min(ma_vals) > 0.10: return None
    ...
    if not tickers:
        ...
        for i in range(0, total_tickers, batch_size):
            ...
            for t, df in data_dict.items():
                ...
                for k in selected:
                    # 改為直接傳入已下載好的 df，不用再去 download_daily
                    try:
